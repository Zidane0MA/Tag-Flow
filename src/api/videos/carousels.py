        ...
        for video in videos:
            ...
            if video_id in video_to_carousel:
                ...
                if base_id in carousel_representatives and video_id == carousel_representatives[base_id]:
                    ...
                    logger.info(f"🎠 CREATING CAROUSEL - base_id:{base_id}, representative:{video_id}, items:{len(carousel_items)}")
                    
                    # Crear el carrusel usando el video actual como base
                    carousel_video = video.copy()
                    carousel_video['is_carousel'] = True
                    carousel_video['carousel_items'] = [item['video_id'] for item in carousel_items]
                    ...
                    logger.info(f"⏭️ SKIPPING CAROUSEL MEMBER - video_id:{video_id}, base_id:{base_id}, not representative")
                    continue
