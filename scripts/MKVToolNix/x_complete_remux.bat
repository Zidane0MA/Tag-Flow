...
:: ==========================================
:: Script de Remuxing MP4 a MKV para TODA la carpeta X
:: Procesa todos los subdirectorios de X (Twitter)
:: ==========================================

:: Configuracion
set "SOURCE_DIR=D:\4K All\Patreon\@BrutalDesu\what-are-you-122066995"
set "MKVMERGE_PATH=C:\Program Files\MKVToolNix\mkvmerge.exe"
set "LOG_FILE=%~dp0x_complete_remux_log_%date:~-4,4%%date:~-10,2%%date:~-7,2%_%time:~0,2%%time:~3,2%%time:~6,2%.txt"

:: Limpiar caracteres especiales del nombre del log
set "LOG_FILE=%LOG_FILE: =0%"

echo ==========================================
echo    REMUXING COMPLETO DE X - Tag-Flow
echo ==========================================
echo.
...
echo Log: %LOG_FILE%
echo.
echo Esto procesara TODOS los subdirectorios de X:
echo - ^&bookmarks
echo - ImmortalsNine
echo - prueba
echo - reprueba
echo - y cualquier otro subdirectorio
echo.
...
:: Inicializar log
echo Inicio del proceso de remuxing completo de X: %date% %time% > "%LOG_FILE%"
echo Directorio fuente: %SOURCE_DIR% >> "%LOG_FILE%"
echo. >> "%LOG_FILE%"
...
echo Contando archivos MP4 en todos los subdirectorios...
for /r "%SOURCE_DIR%" %%f in (*.mp4) do (
    set /a TOTAL_FILES+=1
)

echo Encontrados %TOTAL_FILES% archivos MP4 para procesar en toda la carpeta X
echo.
...
echo en TODOS los subdirectorios de X y eliminara los archivos MP4 originales.
echo.
...
:: Procesar cada archivo MP4 en todos los subdirectorios
for /r "%SOURCE_DIR%" %%f in (*.mp4) do (
    set /a PROCESSED_FILES+=1
    set "MP4_FILE=%%f"
    set "MKV_FILE=%%~dpnf.mkv"
    
    :: Mostrar progreso con subdirectorio
    echo.
                ...
                if exist "!MKV_FILE!" (
                    ...
                    if !MKV_SIZE! GTR 0 (
                        ...
                        if !ERRORLEVEL! EQU 0 (
                            ...
                            echo   EXITOSO: !MP4_FILE! >> "%LOG_FILE%"
                        ) else (
